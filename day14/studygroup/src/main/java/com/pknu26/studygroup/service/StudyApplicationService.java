...
public class StudyApplicationService {
    ...
    public void apply(StudyApplicationForm form) {
        ...
        if (post.getUserId().equals(form.getUserId())) {  // 본인이 작성한 스터디포스트에는 직접 신청할 수 없음
        ...
        if (exists > 0) {  // 신청을 한번 했으면 또 할 수 없음
    ...
    // 260422. 스터디신청 제약을 위해 StudyPost도 전달
    public void approve(Long applicationId, StudyPost post) {
        ...
        // 전체 멤버수를 채우는 신청의 경우는 게시글 포스트 상태를 CLOSED로 변경
        // 신청 승인 로직
