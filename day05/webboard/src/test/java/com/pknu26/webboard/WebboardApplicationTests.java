...
import org.junit.jupiter.api.Assertions.*;  // 추가: JUnit 5 Assertions import
...
class WebboardApplicationTests {
	...
	void testBoardJpa() {
		...
		b1.setContent("첫번째 게시글 입니다. 반갑습니다.");
		b1.setCreateDate(LocalDateTime.now());

		this.boardRepository.save(b1); // INSERT INTO 문

		Board b2 = new Board();
		b2.setTitle("두번째 게시판 글 테스트입니다.");
		b2.setContent("두번째 게시글 입니다.");
		b2.setCreateDate(LocalDateTime.now());

		this.boardRepository.save(b2); // INSERT INTO 문
	...
	void testSelectOneJpa() {
		...
		if (opBoard.isPresent()) { // 
			...
			assertEquals("두번째 게시글 입니다.", board.getContent());  // 두 인자가 같은 값이면 true가 출력.
		}
	}
